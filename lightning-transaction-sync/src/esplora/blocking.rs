...
pub struct BlockingEsploraSyncClient<L: Deref>
where
	L::Target: Logger,
{
...
impl<L: Deref> BlockingEsploraSyncClient<L>
where
	L::Target: Logger,
{
	/// Returns a new [`BlockingEsploraSyncClient`] object.
	...
	/// Returns a new [`BlockingEsploraSyncClient`] object using the given Esplora client.
	...
	pub fn sync(&self, confirmables: Vec<&(dyn Confirm + Sync + Send)>) -> Result<(), TxSyncError> {
		self.inner.sync(confirmables)
	}
}

impl<L: Deref> Filter for BlockingEsploraSyncClient<L>
where
	L::Target: Logger,
{
