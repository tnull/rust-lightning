				...
				match self.get_pending_htlc_info(
					...
					Ok(info) => {
						...
						if pending_add.forward_info.routing.should_hold_htlc() {
							...
							// state to `decode_update_add_htlcs`. If so, the channel's copy of the HTLC will
							// have hold_htlc cleared as a signal, even though the decode-queue clone still has
							// it set.
