		...
		let (tx_opt, chan_opt, shutdown_result) = {
			...
			match peer_state.channel_by_id.entry(msg.channel_id.clone()) {
				hash_map::Entry::Occupied(mut chan_entry) => {
					if let Some(chan) = chan_entry.get_mut().as_funded_mut() {
						let logger = WithChannelContext::from(&self.logger, &chan.context, None);
						let (tx_opt, shutdown_result) = try_channel_entry!(self, peer_state,
							chan.closing_sig(&self.fee_estimator, &peer_state.latest_features, &msg, &&logger),
							chan_entry
						);
						debug_assert_eq!(shutdown_result.is_some(), chan.is_shutdown());

						if let Some(mut close_res) = shutdown_result {
							// We're done with this channel. This also implies there are no pending
							// HTLCs left on the channel, so we can fully delete it from tracking
							// (the channel monitor is still around to watch for old state
							// broadcasts)!
							debug_assert!(tx_opt.is_some());
							let channel = remove_channel_entry!(self, peer_state, chan_entry, close_res);
							(tx_opt, Some(channel), Some(close_res))
						...
						return try_channel_entry!(self, peer_state, Err(ChannelError::close(
							"Got a closing_sig message for an unfunded channel!".into())), chan_entry);
					}
				},
